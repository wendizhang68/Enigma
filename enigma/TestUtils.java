...
class TestUtils {
    ...
    protected static final String TEST1_STRING =
            "DBCAEGFYIJKLMNOPQRSTUVWXHZ";
    protected static final String TEST2_STRING =
            "CBVDFGRHIJKLMNOPQESTUAWXYZ";
    protected static final String TEST3_STRING =
            "GBCDAFRHIJKLMNOPQESTUVWXYZ";

    /**
     * Function used to update rotor arraylist.
     */
    public static void setRotorList(List<Rotor> rotors, Rotor rotor) {
        rotors.add(rotor);
    }

    static Rotor rotorI = new MovingRotor("I",
            new Permutation("(AELTPHQXRU) (BKNW) (CMOY) "
                    + "(DFG) (IV) (JZ) (S)", UPPER), "Q");
    static Rotor rotorII = new MovingRotor("II",
            new Permutation("(FIXVYOMW) (CDKLHUP) (ESZ) "
                    + "(BJ) (GR) (NT) (A) (Q)", UPPER), "E");
    static Rotor rotorIII = new MovingRotor("III",
            new Permutation("(ABDHPEJT) (CFLVMZOYQIRWUKXSG)"
                    + " (N)", UPPER), "V");
    static Rotor rotorIV = new MovingRotor("IV",
            new Permutation("(AEPLIYWCOXMRFZBSTGJQNH)"
                    + " (DV) (KU)", UPPER), "J");
    static Rotor rotorV = new MovingRotor("V",
            new Permutation("(AVOLDRWFIUQ)(BZKSMNHYC) (EGTJPX)", UPPER), "Z");
    static Rotor rotorVI = new MovingRotor("VI",
            new Permutation("(AJQDVLEOZWIYTS) (CGMNHFUX) (BPRK)", UPPER), "ZM");
    static Rotor rotorVII = new MovingRotor("VII",
            new Permutation("(ANOUPFRIMBZTLWKSVEGCJYDHXQ)", UPPER), "ZM");
    static Rotor rotorVIII = new MovingRotor("VIII",
            new Permutation("(AFLSETWUNDHOZVICQ) (BKJ) "
                    + "(GXY) (MPR)", UPPER), "ZM");
    static final Rotor BETA = new FixedRotor("BETA",
            new Permutation("(ALBEVFCYODJWUGNMQTZSKPR) (HIX)", UPPER));
    static final Rotor GAMMA = new FixedRotor("GAMMA",
            new Permutation("(AFNIRLBSQWVXGUZDKMTPCOYJHE)", UPPER));
    static final Rotor B = new Reflector("B",
            new Permutation("(AE) (BN) (CK) (DQ) (FU) (GY) (HW) "
                    + "(IJ) (LO) (MP) (RX) (SZ) (TV)", UPPER));
    static final Rotor C = new Reflector("C",
            new Permutation("(AR) (BD) (CO) (EJ) (FN) (GT) (HK) (IV) (LM) "
            + "(PW) (QZ) (SX) (UY)", UPPER));

    /**
     * Set allRotor1 to be the collection with all naval rotors.
     */
    static List<Rotor> allRotor1 = new ArrayList<>();

    static {
        setRotorList(allRotor1, rotorI);
        setRotorList(allRotor1, rotorII);
        setRotorList(allRotor1, rotorIII);
        setRotorList(allRotor1, rotorIV);
        setRotorList(allRotor1, rotorV);
        setRotorList(allRotor1, rotorVI);
        setRotorList(allRotor1, rotorVII);
        setRotorList(allRotor1, rotorVIII);
        setRotorList(allRotor1, BETA);
        setRotorList(allRotor1, GAMMA);
        setRotorList(allRotor1, B);
        setRotorList(allRotor1, C);
    }
